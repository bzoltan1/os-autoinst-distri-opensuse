...
def isolated_sudoers():
    ...
    This fixture backs up the real sudoers and installs a minimal
    one that only grants access to root itself.

    sudo on SUSE reads "Sudoers path: /etc/sudoers:/usr/etc/sudoers"
    (first found wins). On a fresh install only /usr/etc/sudoers exists.
    If an admin has created /etc/sudoers, it takes precedence.
    """
    # Determine which file sudo will actually read (first-found)
    candidates = ["/etc/sudoers", "/usr/etc/sudoers"]
    sudoers = None
    ...
    backup = sudoers + ".test-backup"
    shutil.copy2(sudoers, backup)
